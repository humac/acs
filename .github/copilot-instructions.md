...
**RBAC Roles:** employee (own assets), manager (all assets read, team management), coordinator (attestation campaigns, read-only assets/users/companies), admin (all access)
...
if (user.role === 'employee') {
  ...
  // Manager, coordinator, and admin see all assets
  assets = await assetDb.getAll();
}

// Coordinator example (campaigns access)
